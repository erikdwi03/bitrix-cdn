...
| **Resize Cache Speed** | 5ms | < 10ms | < 5ms |
| **Throughput** | 850 req/s | > 1000 req/s | > 2000 req/s |
| **P95 Latency** | 185ms | < 200ms | < 100ms |
| **P99 Latency** | 420ms | < 500ms | < 200ms |

### ⚠️ Особенности resize_cache
- **Хранится локально на CDN** - нет сетевых задержек
- **Битрикс пишет через SSHFS** - небольшие задержки на запись
- **WebP генерируется локально** - максимальная скорость
